...
const char DEFAULT_WALLET_FILE_NAME[] = "marcoinwallet.wallet";
const quint64 DEFAULT_OPTIMIZATION_PERIOD = 1000 * 60 * 5; // 5 minutes
const quint64 DEFAULT_OPTIMIZATION_THRESHOLD = 1000000000000; //1H2O target
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "pool.marcoinproject.com:3333";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("marcoinwallet.cfg"));
  ...
  if (cfgFile.open(QIODevice::ReadOnly)) {
   ...
   optimizationObject.insert(OPTION_WALLET_OPTIMIZATION_FUSION_TARNSACTIONS_IS_VISIBLE, true);
    m_settings.insert(OPTION_NODE_REMOTE_RPC_URL, QString("rpc.marcoinproject.com:8607"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("marcoinwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("marcoinwallet.desktop");
  ...
  res = autorunSettings.contains("MarcoinWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("MarcoinWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.marcoin.marcoin scientistwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=Marcoin Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("MarcoinWallet", appPath);
    } else {
      autorunSettings.remove("MarcoinWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\marcoin", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:www.marcoinproject.com");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\marcoin\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\marcoin\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("marcoinwallet.cfg"));
