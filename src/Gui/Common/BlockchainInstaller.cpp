...
BlockchainInstaller::BlockchainInstaller(QObject* _parent) : QObject(_parent), m_blockIndexesFileName("blockindexes.dat"), m_blocksFileName("blocks.dat"),
  m_marcoinDir(Settings::instance().getDataDir().absolutePath()), m_applicationDir(QDir::current()) {
}
...
void BlockchainInstaller::exec() {
  ...
  if (!checkIfMarcoinBlockchainExists()) {
    installBlockchain();
    return;
  }

  quint64 currentHeight;
  quint64 marcoinHeight;
  if (!checkIfBlockchainOutdated(currentHeight, marcoinHeight)) {
    return;
  }
  ...
  QuestionDialog dlg(tr("Blockchain installation"), QString(questionStringTemplate).arg(marcoinHeight).arg(currentHeight), nullptr);
...
bool BlockchainInstaller::checkIfMarcoinBlockchainExists() const {
  return m_marcoinDir.exists() && m_marcoinDir.exists(m_blocksFileName);
}
...
bool BlockchainInstaller::checkIfBlockchainOutdated(quint64& _sourceHeight, quint64& _targetHeight) const {
  ...
  QFile targetBlockIndexesFile(m_marcoinDir.absoluteFilePath(m_blockIndexesFileName));
...
QFileInfo BlockchainInstaller::marcoinBlockchainInfo() const {
  return QFileInfo(m_marcoinDir.absoluteFilePath(m_blocksFileName));
}
...
void BlockchainInstaller::installBlockchain() {
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blockIndexesFileName),
    m_marcoinDir.absoluteFilePath(m_blockIndexesFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blocksFileName), m_marcoinDir.absoluteFilePath(m_blocksFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
